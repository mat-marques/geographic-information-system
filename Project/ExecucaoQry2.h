...
/*
   ...
   comercial identificado por cnpj.Se o estabelecimento existir, então seus dados (nome, descrição do tipo, etc), endereço e coordenada geográfica devem ser impresso em um arquivo texto. Além disso, colocar em um arquivo de extensão '.svg' um círculo (ou retângulo, ou outra forma geométrica) na coordenada geográfica do estabelimento.
 */
...
/*
   ...
   (da respectiva operadora) mais próxima. O usuário está no endereço cep/face/num.
...
/*
   ...
   contenha um estabelecimento do tipo tp e que são do sexo sx (M ou F)?
   SAIDA: arquivo .txt => nome completo e endereço do morador.
 */
...
/*
   ...
   SAIDA: arquivo .txt => imprimir os dados removidos de cada uma da estruturas. Por exemplo:
   RIP: José Silva, portador CPF, do sexo Masculino, nascido à 01/01/1920, residia no endereço CEP/NUM/FACE, usuário da linha móvel NUM da operadora SercomtUEL. arquivo .svg: colocar um losango preto, com um
...
/*
   ...
   identificada por id?
   SAIDA: arquivo .txt=> listar (enumerar) o número dos celulares correntemente ligados à rádio-base.
   arquivo .svg => mostrar no mapa a rádio-base em questão e mostrar a posição dos celulares conectados a ela.
 */
...
/*
   ...
   SAIDA: arquivo .txt => mostra os dados de um cliente por linha
 */
...
/*
   ...
   dentro da região delimitada pelo parâmetros x, y, larg, alt, da, se presente, operadora Sercomtuel (su) ou UELMobile (um).
   SAIDA: arquivo .txt => listar, para cada radio-base, os celulares conectados.
 */
...
/*
   ...
   quadra.
   SAIDA: arquivo .txt =>mostrar cnpj, nome, tipo e endereço de cada estabelecimento comercial.
 */
void executarQryEcq(FILE *arqEntradaQry, FILE **arqSaidaT, Canvas canvas);


/*
   O procedimento executa o comando 'ecr?' do arquivo '.qry'. Esse procedimento realiza a seguinte operação: Lista os estabelecimentos comerciais de tipo de, se presente, uma determinada região.
 */
void executarQryEcr(FILE *arqEntradaQry, FILE **arqSaidaT, Canvas canvas);


/*
   O procedimento executa o comando 'tecq?' do arquivo '.qry'. Esse procedimento realiza a seguinte operação: Lista os tipos de estabelecimentos comerciais de uma dada quadra.
   SAIDA: arquivo .txt =>mostrar, para cada tipo de estabelecimento comercial, listar o nome dos estabelecimentos daquele tipo.
 */
void executarQryTecq(FILE *arqEntradaQry, FILE **arqSaidaT, Canvas canvas);


/*
   O procedimento executa o comando 'tecr?' do arquivo '.qry'. Esse procedimento realiza a seguinte operação: Procura todos os tipos de estabelecimentos comerciais existentes numa dada região.
 */
...
/*
   ...
   SAIDA: arquivo .txt => mostrar dados pessoais
   do dono do celular e o endereço de onde o celular foi conectado pela última vez.
   arquivo .svg => mostrar no mapa a localização do celular
 */
...
/*
   ...
   Semelhante à consulta lec?, porém, reporta a coordenada geográfica da localização do celular.
 */
...
/*
   ...
   SAIDA: arquivo .txt => listar o que foi removido arquivo .svg => elementos removidos não devem aparecer.
 */
